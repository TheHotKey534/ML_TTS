...
def synthesis(text, args, name="test.wav"):
    m = Model()
    m_post = ModelPostNet()
    ...
    write(hp.sample_path + f"/{name}", hp.sr, wav)
    
if __name__ == '__main__':
    
    parser = argparse.ArgumentParser()
    parser.add_argument('--restore_step1', type=int, help='Global step to restore checkpoint', default=24000)
    parser.add_argument('--restore_step2', type=int, help='Global step to restore checkpoint', default=24000)
    ...
    for i in tqdm(range(len(test_samples))):
        synthesis(test_samples[i]["text"],args, f"{i + 1}.wav")
